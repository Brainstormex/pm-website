...
export const PodcastSection = ({ data }: { data: Section }) => {
  ...
  return (
    <section
      className="max-w-7xl mx-auto lg:px-0 px-4 flex justify-between w-full flex-row gap-x-16"
    ...
    >
      ...
      <div className="flex justify-between w-full items-start flex-col border-t border-border pt-0 ">
        ...
        <div className="flex flex-row gap-8 w-full overflow-hidden"> */}
          ...
          {/* Side Podcasts */}
          <div
            className="flex flex-row justify-between overflow-x-auto hide-scrollbar lg:w-full gap-x-8 lg:gap-x-4 lg:justify-between 
            snap-x snap-mandatory scroll-smooth touch-pan-x"
          >
            {data.articles &&
              data.articles.slice(0, 3).map((podcast, index) => (
                <div
                  key={index}
                  className="group cursor-pointer flex flex-col gap-4 snap-start scroll-ml-4 w-[260px]"
                >
                  <div className="relative w-[260px] h-[260px] flex-shrink-0 ">
                    <Link href={podcast.link || "#"}>
                      <Image
                        ...
                        alt={podcast.title}
                        fill
                        className="object-cover rounded-[4px] border-inactiveGray"
                      />
                    </Link>
                  ...
                  <div className="flex-grow">
                    <div className="flex items-center text-sm mb-2">
                      <span className="text-black text-xs">
                        <Link href={podcast.categorySlug || "#"}>
                          {shortenText(podcast.category, 10)}
                        </Link>
                      </span>
                      <span className="mx-2">/</span>
                      <span className="text-description text-xs">
                        <Link href={podcast.authorSlug || "#"}>
                          {podcast.author}
                        </Link>
                      </span>

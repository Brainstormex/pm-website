...
export async function GET() {
    ...
    if (latestSeason) {
        return NextResponse.redirect(new URL(`/podcast/${latestSeason.id}`, process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:8082'));
    }
    // Fallback to first season if no active season is found
    return NextResponse.redirect(new URL(`/podcast/${podcastSeasons[0].id}`, process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:8082'));
} 